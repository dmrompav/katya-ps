...
var horButtons = document.querySelectorAll('.hor-sli__item');
var verSliders = document.querySelectorAll('.ver-sli');
var verButtons = [];

for (var i = 0; i < verSliders.length; i++) {
...
var ModalContent = [[{
  ...
  name: 'О проекте',
  p: 'Данная страница является частью проектной работы на тему "Разработка frontend части web-приложения<br/><br/>Этот сайт является сайтом-резюме, который поможет потенциальному работадателю узнать о моих навыках и умениях, а также связаться со мной.'
}], [{
  name: 'Английский',
...
var x = 0;
var y = [0, 0, 0, 0, 0];
horButtons.forEach(function (btn, ind) {
  btn.addEventListener('click', function () {
    horButtons[x].classList.remove('hor-sli__item--selected');
    horButtons[x].classList.add('hor-sli__item--selectable');
    document.querySelector('.hor-sli').classList.remove("hor-sli--".concat(x));
    document.querySelector('.all-ver').classList.remove("all-ver--".concat(x));
    ...
    document.querySelector('.hor-sli').classList.add("hor-sli--".concat(x));
    document.querySelector('.all-ver').classList.add("all-ver--".concat(x));
